document.addEventListener("DOMContentLoaded", function () {
  ...
  loadStatistics("3"); // Asigură-te că folosești valoarea numerică corectă pentru sezon
});

async function loadStatistics(season) {
  try {
    let url = `/season${season}Statistics`;
    if (season === "all-time") {
      url = `/allTimeStatistics`;
    } else if (season === "all-time-d1") {
      url = `/allTimeStatisticsD1`;
    } else if (season === "all-time-d2") {
      url = `/allTimeStatisticsD2`;
    }
