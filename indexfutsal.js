...
async function populateLeagueTableD2() {
  ...
  if (teams) {
    ...
    // Golirea tabelului înainte de populare
    const tableBodyD2 = document.getElementById("leagueTableBodyD2");
...
async function fetchPlayerStats() {
  try {
    const response = await fetch("/futsalseason1Statistics");
    const data = await response.json();
...
populateGoalsStats(); // Asigură-te că apelezi funcția
...
// Call the function to fetch and display players' assists stats
populateAssistsStats(); // Asigură-te că apelezi funcția
