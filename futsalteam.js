document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`/futsalteam/${encodeURIComponent(teamName)}/`)
    .then((response) => {
      if (!response.ok) {
        throw new Error("Error fetching futsal team details.");
      }
    ...
    .then((data) => {
      ...
      teamNameElement.innerText = team.futsal_team_name;
      logoElement.src = team.futsal_logo;
      foundedSeasonElement.querySelector(
        ".stat-value"
      ).innerText = `FOUNDED IN ${team.futsal_founded_season}`;
      ...
      players.forEach((player) => {
        const careerGoals = player.careerStats.reduce(
          (acc, curr) => acc + curr.futsal_goals,
          0
        );
        const totalGoals = player.futsal_goals + careerGoals;

        const careerAssists = player.careerStats.reduce(
          (acc, curr) => acc + curr.futsal_assists,
          0
        );
        const totalAssists = player.futsal_assists + careerAssists;

        const careerMatches = player.careerStats.reduce(
          (acc, curr) => acc + curr.futsal_matches_played,
          0
        );
        const totalMatches = player.futsal_matches_played + careerMatches;

        const careerCleanSheets = player.careerStats.reduce(
          (acc, curr) => acc + curr.futsal_cleansheets,
          0
        );
        const totalCleanSheets = player.futsal_cleansheets + careerCleanSheets;
        ...
        if (totalGoals > maxGoals) {
          ...
          topScorer = {
            ...
            url: `/futsalprofile.html?id=${player.id}`,
          };
        }
        ...
        row.innerHTML = `
          ...
          <td>${player.futsal_matches_played}</td>
          <td>${player.futsal_goals}</td>
          <td>${player.futsal_assists}</td>
          <td>${player.futsal_cleansheets}</td>
        `;
      ...
      // Fetch and display trophies for a futsal team
  ...
  if (viewPlayersButton) {
    viewPlayersButton.addEventListener("click", () => {
      const teamName = params.get("teamName");
      if (teamName) {
        window.location.href = `/futsalplayers.html?teamName=${encodeURIComponent(
          teamName
        )}`;
      }
    });
